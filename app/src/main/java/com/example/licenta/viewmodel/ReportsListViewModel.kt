    ...
    init {
        ...
        if (userId != null) {
            viewModelScope.launch {
                getReportsStream(userId).collect {
                    _reportsState.value = ReportsState.Success(it)
                }
            }
        } else {
            _reportsState.value = ReportsState.Error("Utilizatorul nu este autentificat.")
        }
    }

    private fun getReportsStream(currentUserId: String) = callbackFlow {
